...
Actuator는 업무 포트가 아니라 별도 포트에서 듣는다(`application-multi-server.properties`).
nginx는 8080만 프록시하므로 `/actuator`는 compose 네트워크 안에서만 닿는다.
...
1. Prometheus `Status > Target health`에서 위 6개 job이 모두 `UP`인지 본다.
...
- **cAdvisor는 이 환경에서 컨테이너 이름을 붙이지 못한다.** 실측으로 확인한 내용이다.
  ...
  `memory.current`, `memory.max`를 읽어 `cgroup_*` 지표로 내보낸다. 호스트 접근이 필요 없고
  다른 지표와 같은 `role`·`node` 라벨을 갖는다. cAdvisor는 JVM이 없는 MySQL·Redis·RabbitMQ·
  nginx용으로만 남겨두었고, 해시 id는 `docker inspect <id 앞부분>`으로 확인한다.
...
첫 기동 검증에서 다섯 인스턴스 모두 CFS throttling이 상시 발생했다. 부하를 걸지 않은
상태에서 스케줄링 주기의 61~78%가 throttle됐다.

| node | throttle된 주기 비율 | 메모리 사용/상한 |
|---|---:|---:|
| web-1 | 61.4% | 46.2% |
| web-2 | 62.0% | 61.3% |
| judge-1 | 73.3% | 79.8% |
