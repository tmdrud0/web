# 대회 제출 저장 경로를 Bulk Insert로 바꿔 처리량을 끌어올린 과정

> 한 줄 요약: 대회 중 가장 먼저 맞는 쓰기 경로를 큐 + batch rewrite 구조로 바꿔, 첫 실패 구간을 최대 2배까지 뒤로 밀었다.
...
아래 그래프는 각 시나리오에서 immediate 방식과 bulk 방식이 "처음 실패를 보인 RPS"를 비교한 것이다.

![First-failure RPS comparison](./images/contest-insert-first-failure-rps.svg)
...
즉 "DB 한 번 쓰는 작업을 조금 모아서 넣는 것"이 아니라, **같은 커넥션과 같은 트랜잭션을 더 짧고 효율적으로 쓰게 만든 것**이 핵심 효과였다.
...
이번 최적화에서 생각보다 컸던 부분은 batch 자체보다도, `merge`와 lazy loading 때문에 붙어 있던 불필요한 조회를 제거한 것이다. 실제로는 "insert 최적화"이면서 동시에 "insert 경로 정리"이기도 했다.
...
즉 batch의 성패는 "DB가 빨라졌는가"보다 "어디에서 얼마나 오래 자원을 붙잡는가"에 더 가깝다.
