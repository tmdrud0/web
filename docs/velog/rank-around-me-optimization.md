# OFFSET 때문에 무너지는 around me 랭킹 조회를 다시 설계한 방법

> 한 줄 요약: 깊은 페이지로 갈수록 느려지는 `OFFSET` 기반 랭킹 조회를, `snapshot rank`와 `solved_count bucket` 중심 구조로 분해했다. current streak는 sub-ms 수준까지 줄였고, solved는 병목을 tie group 내부로 축소했다.
...
즉 단순 페이지 조회보다 "내 위치 계산 + 페이지 fetch"라는 두 문제를 동시에 풀어야 한다.
...
2. bucket에서 "나보다 solved count가 큰 유저 수"를 읽고
3. 같은 solved count 그룹 안에서 내 위치를 계산하고
4. 이를 합쳐 내 rank를 구할 수 있다

즉 `전체 순위 문제`를 `같은 solved count 그룹 내부 문제`로 줄인 셈이다.
...
- 사실상 조회 시점 계산을 없애고, 필요한 rank 구간만 읽는 구조로 바꾼 효과가 그대로 드러났다
...
current streak optimized의 뒤쪽 페이지 실행계획은 거의 "필요한 rank 범위만 스캔"하는 모양이었다.
...
current streak는 rank를 미리 계산하는 방식으로 문제를 거의 제거했고, solved는 bucket으로 범위를 줄여 큰 폭의 개선을 얻었다. 완전히 끝난 작업은 아니지만, 병목을 "전체 데이터셋"에서 "tie group 내부"로 줄였다는 점에서 다음 단계로 나아갈 기반은 충분히 만들었다.
