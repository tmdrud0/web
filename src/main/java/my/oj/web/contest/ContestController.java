...
public class ContestController {
    ...
    public String contestDetail(@PathVariable Long id,
        ...
        List<ContestScoreboardRow> scoreboardRows = buildScoreboardRows(contest.id());
        ...
        model.addAttribute("scoreboardRows", scoreboardRows);
        model.addAttribute("hasScoreboard", !scoreboardRows.isEmpty());
    ...
    private List<ContestScoreboardRow> buildScoreboardRows(Long contestId) {
        List<ContestScoreboardEntry> entries = contestScoreboardService.currentRanking(contestId);
        if (entries.isEmpty()) {
            return List.of();
        }

        Map<Long, String> userNames = userRepository.findAllById(
                        entries.stream().map(ContestScoreboardEntry::userId).collect(Collectors.toSet()))
                .stream()
        ...
        for (ContestScoreboardEntry entry : entries) {
            ...
            rows.add(new ContestScoreboardRow(displayRank, entry.userId(), userName, entry.solvedCount(), entry.penalty()));
        }
        return rows;
    }
}
